...
ErrorCode FeymanMCS10::Init(LibXR::CAN* can,
  ...
  can_ = can;
  config_ = config;
  active_node_id_ = config.connect_node_id;
  running_ = true;
  ...
  if (config_.register_can_callback) {
    can_callback_ = LibXR::CAN::Callback::Create(OnCanFrame, this);
    can_->Register(can_callback_, CAN::Type::STANDARD,
                   CAN::FilterMode::ID_RANGE, 0x180, 0x7FF);
...
ErrorCode FeymanMCS10::Configure() {
  ...
  ec = ConfigureTpdo(
      0, static_cast<uint16_t>(kTpdoIdBase[0] + config_.node_id),
      static_cast<uint8_t>(sizeof(accel_map) / sizeof(accel_map[0])),
      accel_map);
  if (ec != ErrorCode::OK) {
    return ec;
  }
  ...
  ec = ConfigureTpdo(
      1, static_cast<uint16_t>(kTpdoIdBase[1] + config_.node_id),
      static_cast<uint8_t>(sizeof(gyro_map) / sizeof(gyro_map[0])),
      gyro_map);
  if (ec != ErrorCode::OK) {
    return ec;
  }
  ...
  if (config_.connect_node_id != config_.node_id) {
    ec = WriteTargetNodeId();
    if (ec != ErrorCode::OK) {
      return ec;
    }
  }

  const bool readdressing = (config_.connect_node_id != config_.node_id);
  ec = readdressing ? SendNmt(kNmtResetCommunication, 0U)
                    : SendNmt(kNmtResetCommunication);
  if (ec != ErrorCode::OK) {
    return ec;
  }
  if (readdressing) {
    active_node_id_ = config_.node_id;
  }
  LibXR::Thread::Sleep(50U);

  return readdressing ? SendNmt(kNmtStartRemoteNode, 0U)
                      : SendNmt(kNmtStartRemoteNode);
}

ErrorCode FeymanMCS10::ReadNodeId(uint8_t* node_id_out) {
  if (node_id_out == nullptr) {
    return ErrorCode::PTR_NULL;
  }

  uint32_t value = 0U;
  const ErrorCode ec = SdoReadU32(kObjNodeId, 0x00U, &value);
  if (ec != ErrorCode::OK) {
    return ec;
  }

  *node_id_out = static_cast<uint8_t>(value & 0xFFU);
  return ErrorCode::OK;
}

ErrorCode FeymanMCS10::Readdress() {
  if (can_ == nullptr || !running_) {
    return ErrorCode::INIT_ERR;
  }
  if (config_.connect_node_id == config_.node_id) {
  ...
  ec = SendNmt(kNmtResetCommunication, 0U);
  if (ec != ErrorCode::OK) {
    return ec;
  }
  active_node_id_ = config_.node_id;
  ...
  ec = SendNmt(kNmtStartRemoteNode, 0U);
  if (ec != ErrorCode::OK) {
    return ec;
  }
  LibXR::Thread::Sleep(20U);
  ...
  uint8_t readback = 0U;
  ec = ReadNodeId(&readback);
  if (ec != ErrorCode::OK) {
    return ec;
  }
  return (readback == config_.node_id) ? ErrorCode::OK : ErrorCode::CHECK_ERR;
}
...
void FeymanMCS10::ProcessCanFrame(bool in_isr,
                                  const LibXR::CAN::ClassicPack& pack) {
  HandleCanFrame(in_isr, pack);
}
...
void FeymanMCS10::HandleCanFrame(bool in_isr,
  ...
  const uint32_t node = active_node_id_;
...
ErrorCode FeymanMCS10::WriteTargetNodeId() {
  if (config_.connect_node_id == config_.node_id) {
    return ErrorCode::OK;
  }
  return SdoWriteU8(kObjNodeId, 0x00U, config_.node_id);
}
...
ErrorCode FeymanMCS10::SendNmt(uint8_t command) {
  return SendNmt(command, active_node_id_);
}

ErrorCode FeymanMCS10::SendNmt(uint8_t command, uint8_t node_id) {
  const uint8_t payload[2] = {command, node_id};
...
ErrorCode FeymanMCS10::SendSdoRequest(
  ...
  return SendCanFrame(kSdoReqIdBase + active_node_id_, frame.data(), 8U,
                      CAN::Type::STANDARD);
}
